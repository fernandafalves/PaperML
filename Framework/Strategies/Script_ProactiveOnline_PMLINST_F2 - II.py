def fmain(value, variation):
    ...
    for time_period in range(1,3):
        if time_period == 1:
            ampl.eval('reset;')
            ampl.read('/Approaches/Proactive-Online approaches/Data_ProactiveOnline_PMLINST_F2.txt')
        ampl.eval('let first_iteration:= 1;')
        ampl.eval('let subperiod:= 0;')
        ...
        ampl.read('/Approaches/Proactive-Online approaches/ProactiveOnline_PMLINST_F2.txt')

        retrain = ampl.getParameter("retrain")
        subperiod = ampl.getParameter("subperiod")
        counter_retrain = ampl.getParameter("counter_retrain")

        retrain = retrain.value()
        subperiod = subperiod.value()
        ...
        while retrain == 1:
            ...
            if time_period == 1:
                for period in range(112 - int(subperiod)):
                    if pred_M1[period] == 1:
                        MOMENTS_1.append(int(subperiod) + period + 1)  
                    
                    if pred_M2[period] == 1:
                        MOMENTS_2.append(int(subperiod) + period + 1)
            else:
                for period in range(112 - int(subperiod)):
                    if pred_M1_2[period] == 1:
                        MOMENTS_1.append(int(subperiod) + period + 1)
    ...
    times = open("/Approaches/Proactive-Online approaches/Time_PMLINST_F2.txt","a") 
